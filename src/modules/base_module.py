...
class BaseModule(ABC):
    ...
    async def _call_gemini(self, expression: str, **prompt_kwargs) -> Dict[str, Any]:
        ...
        """
        prompt = self.domain_prompt.format(expression=expression, **prompt_kwargs)

        return await self.gemini_agent.generate_json_response(prompt)

    def _create_result(
        self, gemini_response: Dict[str, Any], domain: str
    ) -> CalculationResult:
