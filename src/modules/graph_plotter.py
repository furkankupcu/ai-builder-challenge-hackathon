...
class GraphPlotterModule(BaseModule):
    ...
    async def _create_plot(
        self, visual_data: Dict[str, Any], expression: str
    ) -> Dict[str, str]:
        """Grafik olusturur
    ...
    async def _plot_2d(
        self, visual_data: Dict[str, Any], expression: str, x_range: list
    ) -> Dict[str, str]:
        """2D grafik cizer"""
        try:
            x = np.linspace(x_range[0], x_range[1], 1000)

            # Basit eval ile fonksiyonu hesapla
            # Guvenlik icin sadece numpy fonksiyonlarina izin ver
            allowed_names = {
            ...
            try:
                ...
                logger.warning(
                    f"Eval error for '{expression}': {eval_error}. Fallback to x^2"
                )
                y = x**2
